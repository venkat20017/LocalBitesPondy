...
export const HeroB = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            await saveLeadToSheets({ name, email, source: 'hero_b_signup' }); // Updated source
            ...
            trackConversion('hero_signup'); // Keep same conversion event or make unique if needed
            setIsSuccess(true);
            setName('');
            setEmail('');
        } catch (error) {
                ...
                <div className="w-full md:w-1/2 flex flex-col justify-center px-6 py-24 md:px-12 lg:px-20 z-10 bg-white">
                    <div id="download" className="animate-fade-in-up">
                        <h1 className="mb-6 text-4xl font-extrabold tracking-tight text-gray-900 md:text-5xl lg:text-6xl leading-tight">
                            Famous Foods of <span className="text-orange-500">Pondicherry</span>
                        </h1>
                        <p className="mb-10 text-lg text-gray-600 md:text-xl leading-relaxed">
                            Pondicherry (also known as Puducherry) is one of India’s most distinctive culinary destinations. The city’s food culture is shaped by a rare blend of Tamil heritage and French colonial influence.
                        </p>
