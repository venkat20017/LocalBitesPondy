...
export default function RestaurantDetail() {
  ...
  if (loading && !r) {
    return (
      <>
        <PageSEO title="Loading restaurant…" canonicalPath={`/restaurants/${slug ?? ''}`} noindex />
    ...
    return (
      <>
        <PageSEO
          ...
          canonicalPath={`/restaurants/${slug ?? ''}`}
          noindex
        />
  ...
  const seoTitle = r.seo?.metaTitle ?? `${r.name}${r.location ? ` — ${r.location}` : ''}`;
  const seoDescription =
    r.seo?.metaDescription ??
    r.shortDescription ??
    `${r.name} in ${r.location ?? 'Pondicherry'}. ${cuisines.join(', ')}. Hand-picked & vetted by LocalBitesPondy.`;
  const ogImg = imageToOgUrl(r.seo?.ogImage) ?? imageToOgUrl(r.heroImage);
  ...
  const lds = [
    ...
    breadcrumbJsonLd([
      ...
      { name: r.name ?? 'Restaurant', path: `/restaurants/${r.slug?.current ?? slug ?? ''}` },
    ]),
  ];
  ...
  return (
    <>
      <PageSEO
        ...
        canonicalPath={`/restaurants/${r.slug?.current ?? slug ?? ''}`}
        ogImage={ogImg}
        ogType="article"
